...
export const ShowForm: React.FC<ShowFormProps> = ({ show, onBack, onSaved }) => {
  ...
  const handleSaveShow = async () => {
    ...
    try {
      ...
      setSaveError(typeof detail === 'string' ? detail : Array.isArray(detail) ? detail.map((e: any) => e.msg || JSON.stringify(e)).join('; ') : JSON.stringify(detail) || 'Failed to save show.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateEpisode = async () => {
    ...
    try {
      await api.post('/episodes/season/' + selectedSeasonId, {
        episode_number: (show?.seasons?.find(s => s.id === selectedSeasonId)?.episodes?.reduce((max: number, ep: any) => Math.max(max, ep.episode_number), 0) || 0) + 1,
        title: newEpTitle,
      ...
      setNewEpTitle('');
      setNewEpDuration(300);
      setNewEpContentGroup('');
      setNewEpLanguage('en');
      setNewEpThumbUrl('');
      onSaved();
    } catch (err: any) {
      const epDetail = err.response?.data?.detail;
      setEpError(typeof epDetail === 'string' ? epDetail : Array.isArray(epDetail) ? epDetail.map((e: any) => e.msg || JSON.stringify(e)).join('; ') : JSON.stringify(epDetail) || 'Failed to add episode.');
    }
  };
  ...
  const handleDeleteEpisode = async (episodeId: string) => {
    if (window.confirm('Delete this episode track from the show?')) {
      try {
        ...
        const delDetail = err.response?.data?.detail;
        alert(typeof delDetail === 'string' ? delDetail : Array.isArray(delDetail) ? delDetail.map((e: any) => e.msg || JSON.stringify(e)).join('; ') : JSON.stringify(delDetail) || 'Failed to delete episode track.');
      }
    }
  };
