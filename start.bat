...
start "Peblo TV - CMS Studio" cmd /k "cd /d "%ROOT_DIR%cms-ui" && if not exist node_modules (call npm install) & call npm run dev -- --port 3001"

echo [3/3] Starting Viewer OTT UI on Port 3000...
start "Peblo TV - Viewer UI" cmd /k "cd /d "%ROOT_DIR%viewer-ui" && if not exist node_modules (call npm install) & call npm run dev -- --port 3000"
