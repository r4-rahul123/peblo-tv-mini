...
async def generate_validation_report(db: AsyncSession) -> ValidationReport:
    ...
    for show in shows:
        ...
        if is_show_published and show_published_episodes == 0:
            show_issues.append(
                ValidationIssue(
                    entity_type="show",
                    entity_id=show.id,
                    ...
                    severity="BLOCKER",
                    field="episodes",
                    message="Published show must have at least one published episode.",
                    action_required="Publish at least one episode or unpublish the show.",
                )
            )
            total_blockers += 1
