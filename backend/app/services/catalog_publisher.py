...
from app.models.models import Episode, PublishRun, Season, Show
...
async def publish_catalog(db: AsyncSession, triggered_by: str) -> dict[str, Any]:
        ...
        for season in sorted(show.seasons, key=lambda s: s.season_number):
            ...
            if is_season_trailer:
                for cg, ep_list in grouped_episodes.items():
                    default_ep = next(
                        (e for e in ep_list if e.language == "en"), ep_list[0]
                    )
    ...
    sorted_sections = [
        {"name": sec_name, "section_name": sec_name, "shows": shows_in_sec}
    ...
    catalog_json_str = json.dumps(
        published_catalog_payload, indent=2, ensure_ascii=False
    )
...
async def rollback_to_run(
    db: AsyncSession, target_run_id: str, triggered_by: str
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        res = await db.execute(
            select(PublishRun).where(PublishRun.run_id == target_run_id)
        )
        target_run = res.scalar_one_or_none()
        if not target_run:
            return {
                "success": False,
                "message": f"Run {target_run_id} not found in history.",
            }
        return {
            "success": False,
            "message": f"Snapshot file for run {target_run_id} not available on disk.",
        }
