...
async def search_catalogue(
    ...
    if cache_key in _catalog_cache and (now - _catalog_cache[cache_key][0]) < _CACHE_TTL:
        catalog = _catalog_cache[cache_key][1]
    else:
        try:
            data = await storage.read_file(CATALOG_DESTINATION_PATH)
            catalog = json.loads(data.decode("utf-8"))
            _catalog_cache[cache_key] = (now, catalog)
        except FileNotFoundError:
            return {
                "results": [],
                "total": 0,
...
async def trigger_rollback(
    ...
    if not re.match(r"^[a-zA-Z0-9_\-]+$", run_id):
        raise HTTPException(
            status_code=400,
            detail="Invalid run_id format. Only alphanumeric characters, hyphens, and underscores allowed.",
        )
