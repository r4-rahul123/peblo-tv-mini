...
# Create .env with DEBUG enabled for local development
echo DEBUG=true > .env
...
### Option C: 1-Click Start on Windows (`start.bat`)

Double-click `start.bat` or run from root terminal:
```cmd
start.bat
```
*(Automatically starts FastAPI Backend, CMS Studio, and Viewer UI in separate terminal windows).*
...
| `SECRET_KEY` | *(auto-generated)* | Cryptographic secret key used for JWT token signing. Auto-generates a secure random key if not set. **Must be set explicitly in production.** |
...
| `DATABASE_URL` | `sqlite+aiosqlite:///./backend/peblo.db` | Async SQLAlchemy database connection string (SQLite for local dev; `postgresql+asyncpg://<user>:<password>@<host>:5432/<dbname>` for production/Docker). |
...
| `BACKEND_CORS_ORIGINS` | `["http://localhost:3000","http://localhost:3001","http://localhost:8000"]` | JSON list of allowed origins permitted to make cross-origin requests to the FastAPI backend. |
| `DEBUG` | `false` | Set to `true` for local development to enable the `/token` endpoint and `X-User-Role` header for seamless CMS role switching. **Must be `false` in production.** |
| `VITE_BACKEND_URL` | `http://localhost:8000` | Backend API URL used by both frontend apps (CMS and Viewer). Set in frontend `.env` files. |
